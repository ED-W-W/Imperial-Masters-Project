...
def plot_smax_metrics_multi_rnn(results_dict, save_prefix="iql_training_metrics_rnn_comparison"):
    """
    ...
    os.makedirs("iql_results", exist_ok=True)
    ...
    for hidden_size, df in results_dict.items():
        if "test_returned_won_episode" in df.columns:
            ...
            df[["env_step", "test_returned_won_episode"]].to_csv(
                f"iql_results/{save_prefix}_win_rate_rnn{hidden_size}.csv",
                index=False
            )
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"iql_results/{save_prefix}_win_rate.png", dpi=300)
    plt.show()
    ...
    for hidden_size, df in results_dict.items():
        if "test_returned_episode_returns" in df.columns:
            ...
            df[["env_step", "test_returned_episode_returns"]].to_csv(
                f"iql_results/{save_prefix}_return_rnn{hidden_size}.csv",
                index=False
            )
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"iql_results/{save_prefix}_mean_return.png", dpi=300)
    plt.show()
...
if __name__ == "__main__":
    ...
    for h in hidden_sizes:
        ...
        os.makedirs("iql_results/raw_metrics", exist_ok=True)
        metrics_df.to_csv(f"iql_results/raw_metrics/iql_training_metrics_rnn{h}_seed{seed}.csv", index=False)
